# Plugin „Temperatur“

<img src="{{site.baseurl}}/plugin-temperature/{{site.img}}/temperature_icon.png" class="pluginLogo" width="100" />

## Info zum Änderungsprotokoll

### Wichtig

> **_Zur Erinnerung_**: Wenn keine Informationen zum Update vorliegen, bedeutet dies, dass es sich ausschließlich um Aktualisierungen der Dokumentation, Übersetzungen oder Korrekturen kleinerer Fehler handelt.

## 2026

### 06/03/2026

- Verbesserung und Aktualisierung des Plugins

## 2025

### 19/04/2025

- GitHub-Plugin-Migration

### 06/02/2025

- Link zur Dokumentation aktualisiert

### 15-16/01/2025

- Hinzufügen einer Steuerung für die Windgeschwindigkeit

### 14/01/2025

- Korrektur der Berechnung der gefühlten Temperatur: https://community.jeedom.com/t/erreur-calcul-de-la-temperature-ressentie/136577?u=jag

## 2024

### 28/12/2024

- Tippfehler
- Verbesserung des Protokolls

### 27/09/2024

- Behebung eines Fehlers bei der Installation über den Market

### 20/09/2024

- Übersetzung

### 13/09/2024

- Verbesserung des Log-Plugins
- Wiederaufnahme der Auftragserstellung
- Behebung eines Fehlers beim Update (2 gleichzeitig aktive Cron-Jobs)
- Änderung des Namens des Befehls „vent“
- Übersetzung mit Dank an @Mips
- Korrektur der PHP8-Warnung

> **Achtung: Dieses Update löscht den Befehl „message“; er wird nach dem Update automatisch neu erstellt.**


### 04/02/2024

- Entfernung des Community-Links nach der Umstellung auf Core 4.4
- Verbesserung der Suchleiste

## 2023

### 08/10/2023

- Verbesserung der Informationen für die Community zu Core 4.4
- Tippfehler

### 02/04/2023

- Korrektur für Core 4.4
- Die Mini-Core-Version für das Plugin ist 4.2

### Version 20230327

- Tippfehler

## 2022

### Version 20220909

- Neue Funktion in Core V4.3

### Version 20220205

- Core-Anzeige v4.2

## 2021

### Version 20210728

- Core v4.2 (Beta) – Tabellarische Anzeige
- Korrektur des übergeordneten Objekts
- Allgemeine Verbesserung der Anzeige

## 2020

### Version 20201208

- Behebung eines Fehlers bei der Humidex-Berechnung
- Behebung verschiedener Fehler

### Version 20201129

- Verbesserung der Anzeige, Hinzufügen von Tooltips zu den Befehlen
- Verbesserung und Aktualisierung der Steuerung

### Version 20201027

- Verbesserung der visuellen Darstellung auf dem Dashboard
- Verbesserung der Auftragserstellung
- Verbesserung und Aktualisierung der Steuerung
- Anzeige der Befehle ändern
- BP-Reset zur Suche hinzugefügt
- Clean Log + Code
- Behebung eines Fehlers beim Löschen des Befehls „Refresh“
- PHP-Warnung beheben
- Variable „correction“ nicht definiert
- Behebung eines Fehlers beim Anlegen von Befehlen
- Verbesserung der Liste der übergeordneten Objekte

### Version 20200523

- Hinzufügen eines zusätzlichen Protokolls im DEBUG-Modus
- Reinigung des Protokolls
- Schaltfläche zum erneuten Erstellen der Befehle hinzugefügt
- Änderung bei der Erstellung von Bestellungen
- Behebung des Cron-Fehlers
- Verbesserung der Anzeige V4
- Aktualisierung der Dokumentation
- Hinzufügen eines Core-Widgets zu den Steuerelementen
- Behebung eines Fehlers bei der individuellen Registrierung jedes Geräts
- Registrierung der Geräte nach jedem Update
- Änderungen an der Dokumentation

> Hinweis: Denken Sie daran, jedes Gerät zu sichern

## Sonstiges

### Version 3.1.1

- Korrektur der Abbildungen in der Dokumentation

### Version 3.1

- Hinzufügen eines Cron-Jobs 30
- Verbesserung der Anzeige für den Core V4
- Möglichkeit, Befehle umzubenennen
- Befehl „Aktualisieren“ (auf der Kachel, im Szenario usw.)
- Verbesserung der Protokolle
- Korrektur des Abspanns
- Fehlerbehebung: Die Datenaktualisierung findet nicht mehr statt, wenn das Gerät deaktiviert ist
- Ordner bereinigen
- Hinzufügen eines Voralarms für den Humidex-Grenzwert
- Übernahme der Humidex-Werte
- Korrektur der Dokumentation

> *Wichtig: Die Geräte müssen neu angelegt werden, damit der Humidex-Voralarm hinzugefügt wird.
> *Hinweis: Es wird empfohlen, das Plugin zu deinstallieren und anschließend neu zu installieren\*

### Version 3.0

- Unterstützung für PHP 7.3
- Umstellung auf Font Awesome 5
- Umstellung der Anzeige auf das Core-V4-Format

### Version 2.0

- Update zur Kompatibilität mit Jeedom V3

### Version 1.1

- Korrekturlesen
