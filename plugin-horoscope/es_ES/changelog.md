# Complemento Horóscopo

<img src="{{site.baseurl}}/plugin-horoscope/{{site.img}}/horoscope_icon.png" class="pluginLogo" width="100" />

## Información sobre el historial de cambios

### Importante

> **_Recordatorio_**: si no hay información sobre la actualización, es porque esta se refiere únicamente a la actualización de la documentación, a traducciones o a correcciones de errores menores.

## 2026

### 07/07/2026

- Cambio en las fuentes de los horóscopos (gracias a Noodom)

### 08/03/2026

- Cambio de las fuentes de los horóscopos (se han modificado todos los comandos)
- Incorporación de horóscopos diarios y semanales

## 2025

### 25/01/2025

- Actualización del enlace a la documentación

## 2024

### 02/11/2024

- Problema al crear consultas [https://community.jeedom.com/t/debian-12-et-jeedom-4-4-19/133047](https://community.jeedom.com/t/debian-12-et-jeedom-4-4-19/133047)

### 01/11/2024

- Errores tipográficos

### 22/09/2024

- Traducción

> **Atención: la traducción solo se ha realizado en la interfaz; el texto original de los horóscopos está únicamente en francés**

### 13/09/2024

- Corrección de PHP 8
- Traducción

### 05/09/2024

- Corrección de PHP 8

### 21/08/2024

- Traducción
- Typo Log core 4.4
- Mejoras en Log core 4.4

### 04/02/2024

- Eliminación del enlace a la comunidad tras el cambio al núcleo 4.4
- Mejora de la barra de búsqueda

### 14/01/2024

- Mejoras para Core V4.4

## 2023

### 16/12/2023

- Añadir tipo de horóscopo (normal o resumido)
- Modificación de la creación de pedidos
- Posibilidad de eliminar los comandos

### 08/10/2023

- Mejora de la información para la comunidad en Core 4.4
- Errores tipográficos
- Cambio del subtipo de comando para el número de oportunidades en formato numérico

### 02/06/2023

- Recuperación del complemento
- La versión mini Core del complemento es la 4.2
